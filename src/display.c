...
static mp_obj_t lvgl_esp32_Display_mirrorX(mp_obj_t self_ptr,mp_obj_t en)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_mirror(self->panel, mp_obj_is_true(en), self->mirror_y));
...
static mp_obj_t lvgl_esp32_Display_mirrorY(mp_obj_t self_ptr,mp_obj_t  en)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_mirror(self->panel, self->mirror_x, mp_obj_is_true(en)));
...
static mp_obj_t lvgl_esp32_Display_swapXY(mp_obj_t self_ptr,mp_obj_t  en)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_swap_xy(self->panel, mp_obj_is_true(en)));
...
static void clear(lvgl_esp32_Display_obj_t *self)
{
    ...
    uint8_t *buf = heap_caps_calloc(1, buf_size * 16, MALLOC_CAP_DMA);
