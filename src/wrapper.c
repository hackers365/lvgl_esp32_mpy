...
static mp_obj_t lvgl_esp32_Wrapper_init(mp_obj_t self_ptr)
{
    ...
    self->buf1 = heap_caps_malloc(self->buf_size * sizeof(lv_color_t), MALLOC_CAP_DMA|MALLOC_CAP_SPIRAM);
    assert(self->buf1);
    self->buf2 = heap_caps_malloc(self->buf_size * sizeof(lv_color_t), MALLOC_CAP_DMA|MALLOC_CAP_SPIRAM);
    assert(self->buf2);
